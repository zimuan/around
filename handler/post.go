...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
    ...
    user := r.Context().Value("user") // 返回的是user提供的token
    claims := user.(*jwt.Token).Claims // token 里的payload
    ...
    if err != nil {
        http.Error(w, "Failed to read media_file from request", http.StatusBadRequest)
        fmt.Printf("Failed to read media_file from request %v.\n", err)
        return
    }
...
func searchHandler(w http.ResponseWriter, r *http.Request){
    // 通过url传user， Request提供了如何从url拿到user数据的function
